...
from a2_skills import MageAttack, MageSpecial, RogueAttack, RogueSpecial, \
    VampireAttack, VampireSpecial, SorcererSpecial, SorcererAttack
...
class Sorcerer(Character):
    """
    A class representing a Socrcerer.
    ...
    enemy - the charecter that this Sorcerer attacks.
    """
    ...
    def attack(self) -> None:
        ...
        self._skills['A'] = \
            SorcererAttack\
                (self.default.pick_skill(self, self.enemy))
        real_depletion = self.get_sp() - 15

        self._current_state = 'attack'
        self._current_frame = 0
        self._skills['A'].use(self, self.enemy)

       # self.set_sp(real_depletion) if real_depletion > 0 else self.set_sp(0)

        if real_depletion > 0:
            self.set_sp(real_depletion)
        else:
            self.set_sp(0)

    def set_skill_decision_tree(self, default: 'SkillDecisionTree') -> None:
        """
        sets the skill decesion tree
        >>> from a2_battle_queue import BattleQueue
        >>> from a2_playstyle import ManualPlaystyle
        >>> bq = BattleQueue()
        >>> c = Sorcerer("r", bq, ManualPlaystyle(bq))
        ...
        >>> bq.add(c)
        >>> bq.add(c2)
        >>> m = create_default_tree()
        >>> c.set_skill_decision_tree(m)
        >>> c.default == m
        True
...
class Vampire(Character):
    ...
    enemy - the charecter that this Vampire attacks.
    """
    ...
    def copy(self, new_battle_queue: 'BattleQueue') -> 'Charecter':
        """
        ...
        >>> from a2_playstyle import ManualPlaystyle
        >>> bq = BattleQueue()
        >>> p = Vampire("r", bq, ManualPlaystyle(bq))
        >>> p2 = Vampire("r2", bq, ManualPlaystyle(bq))
        >>> p.enemy = p2
        >>> p2.enemy = p
        >>> p
        r (Vampire): 100/100
